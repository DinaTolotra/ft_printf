...
/*   Updated: 2026/02/10 08:09:35 by todina-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_putnb_base_rec(uintptr_t nb, const char *b, int b_val)
{
	int	count;

	count = 1;
	if (nb >= (unsigned int)b_val)
		count += ft_putnb_base_rec(nb / b_val, b, b_val);
...
int	ft_putnb_base(uintptr_t nb, const char *b)
{
	...
	if (b_val == 0)
		return (0);
	return (count + ft_putnb_base_rec(nb, b, b_val));
}
